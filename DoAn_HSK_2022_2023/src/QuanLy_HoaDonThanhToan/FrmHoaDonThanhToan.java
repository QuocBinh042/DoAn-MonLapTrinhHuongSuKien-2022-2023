...
public class FrmHoaDonThanhToan extends JFrame implements ActionListener, MouseListener {
	...
	private DatabaseHoaDonThanhToan databasee = new DatabaseHoaDonThanhToan();
	...
	public void loadData() {
		try {
			ds = databasee.read_HoaDonThanhToan("QuanLy_HoaDonThanhToan.txt");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		if (ds == null) {
			ds = new DanhSachHoaDonThanhToan();
		} else {
			for (HoaDonThanhToan hd : ds.getList()) {
				String[] row = { hd.getMaHoaDon(), hd.getNgayThanhToan(), hd.getHinhThucThanhToan(),String.valueOf(hd.getTongThanhToan()),
						hd.getGhiChu() };
	...
	private void Luu() {
		...
		try {
			...
			databasee.write_HoaDonThanhToan("QuanLy_HoaDonThanhToan.txt", ds);
		} catch (Exception e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
	...
	private void XoaHoaDonThanhToan() throws Exception {
		...
		if (r != -1) {
			...
			if (tb == JOptionPane.YES_OPTION) {
				...
				xoaTrang();
				databasee.write_HoaDonThanhToan("QuanLyHoaDonThanhToan.txt", ds);
			}
		} else {
